...
import { getChatbotSettings, updateChatbotSettings, getFaqs, saveFaqs } from '../../../api/aiChatApi';
...
const FAQ_COLLECTION = 'chatbot-faqs';
const DEFAULT_FAQS = [
...
const FaqModal = ({ isOpen, onClose, onSave, faq, isSaving }) => {
  ...
  const handleSave = () => {
    onSave({ question, answer, keywords: keywords.split(',').map(k => k.trim()).filter(Boolean) });
  };
...
// 시간 설정 모달 컴포넌트
const TimeSettingModal = ({ isOpen, onClose, onSave, initialTimeRange, isSaving }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm transform transition-all border">
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-900">AI 자동 답변 시간</h2>
        
        <div className="space-y-5">
          <div>
            <label htmlFor="startTimeModal" className="block text-sm font-semibold text-gray-700 mb-1">시작 시간</label>
          ...
          <div>
            <label htmlFor="endTimeModal" className="block text-sm font-semibold text-gray-700 mb-1">종료 시간</label>
             <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                     <svg className="w-5 h-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
        ...
        <div className="grid grid-cols-2 gap-3 mt-8">
          <button onClick={onClose} className="w-full px-4 py-3 bg-gray-100 text-gray-800 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
            취소
          </button>
          <button onClick={handleSave} disabled={isSaving} className="w-full px-4 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-blue-300 transition-colors">
            {isSaving ? "저장중..." : "저장"}
          </button>
        </div>
      </div>
...
const ChatbotSettings = () => {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      setAnswerTimeRange(res.answerTimeRange || { start: '09:00', end: '18:00' });
      // 서버에서 받은 시간 설정으로 초기화
  ...
  // 로딩
  if (isLoading || hospitalLoading) return <div className="p-6">로딩 중...</div>;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => handleOpenFaqModal()} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
              질문 추가
            </button>
          </div>
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div key={faq.id} className="p-4 border rounded-md">
                <div>
                  ...
                  {faq.keywords && faq.keywords.length > 0 && (
                    <div className="mt-2 flex items-center flex-wrap gap-1">
                      <span className="text-xs font-semibold mr-1">키워드:</span>
                ...
                <div className="flex items-center space-x-2 mt-3 justify-end">
                  <button onClick={() => handleOpenFaqModal(faq)} className="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-100">
                    편집
                  </button>
           ...
           <div className="mt-6 border-t pt-6">
             ...
             <div>
                <p className="text-gray-600">
                  <span className="font-mono bg-gray-100 px-2 py-1 rounded">{timeRange.start}</span> 부터 
                  <span className="font-mono bg-gray-100 px-2 py-1 rounded">{timeRange.end}</span> 까지
                </p>
                <button onClick={() => setIsTimeModalOpen(true)} className="mt-4 w-full px-4 py-2 bg-gray-800 text-white rounded-md hover:bg-gray-900">
                  시간대 변경
                </button>
             </div>
           </div>
      ...
      <FaqModal isOpen={isFaqModalOpen} onClose={handleCloseFaqModal} onSave={handleSaveFaq} faq={editingFaq} isSaving={isSaving} />
      <TimeSettingModal 
