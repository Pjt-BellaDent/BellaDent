...
function formatDateOnly(dateStr) {
  ...
  if (dateStr && typeof dateStr === 'object' && typeof dateStr._seconds === 'number') {
        ...
        <tbody className="divide-y divide-gray-100 text-center">
          {data && data.length > 0 ? data.map((p, idx) => (
            <tr
              ...
              className={
                `${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 transition-colors`
              }
            >
              <td>
                <span
              ...
              <td>
                {p.gender === '남' || p.gender === 'M' ? (
                  <span className="inline-block px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs font-bold">남</span>
                ) : p.gender === '여' || p.gender === 'F' ? (
                  <span className="inline-block px-2 py-0.5 rounded bg-pink-100 text-pink-700 text-xs font-bold">여</span>
                ) : (
                  <span className="inline-block px-2 py-0.5 rounded bg-gray-200 text-gray-700 text-xs font-bold">-</span>
                )}
              </td>
              <td>{p.birth}</td>
              <td><span className="text-gray-700 font-mono">{formatPhone(p.phone)}</span></td>
