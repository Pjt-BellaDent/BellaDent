// src/components/app/patients/ProcedureModal.jsx
...
const ProcedureModal = ({
  ...
  const reservedTimes = useMemo(() => {
    if (
      !formData.department ||
      !formData.date ||
      !Array.isArray(events[formData.date])
    )
      return [];
    return events[formData.date]
      ...
      .flatMap((e) => {
        ...
        if (e.time.includes('~')) {
          ...
          return HOUR_MAP.slice(
            HOUR_MAP.indexOf(start),
            HOUR_MAP.indexOf(end) + 1
          );
        }
  ...
  useEffect(() => {
    ...
    axios
      .get(`/procedures/name/${patient.name}/${patient.birth}`)
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h3 className="text-xl font-bold mb-4">
          {patient?.name} ({patient?.birth}) 시술 이력
        </h3>
          ...
          <ul className="border-l-4 border-blue-500 pl-4 space-y-4">
            {procedures.map((p, i) => (
              <li key={i} className="relative">
                ...
                {p.memo && (
                  <p className="bg-gray-50 text-sm mt-2 p-2 rounded">
                    {p.memo}
                  </p>
                )}
              </li>
            ...
            {formData.department && (
              <>
                ...
                >
                  ...
                  {DOCTOR_MAP[formData.department].map((doc) => (
