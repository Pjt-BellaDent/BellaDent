      ...
      try {
        ...
        new Chart(reservationRef.current, {
          ...
          data: {
            ...
            datasets: [{ label: '예약 수', data: stats.reservations, fill: false, borderColor: '#007bff', tension: 0.3 }]
          },
        ...
        new Chart(procedureRef.current, {
          ...
          data: {
            ...
            datasets: [{ label: '시술 횟수', data: stats.procedures, backgroundColor: ['#007bff', '#28a745', '#ffc107'] }]
          },
          options: { responsive: true, plugins: { legend: { display: false } } }
        });
      } catch (error) {
  ...
  return (
    <div className="p-6 font-sans">
      ...
      <div className="flex gap-4 items-center mb-6">
        <label>진료과:</label>
        <select value={selectedDept} onChange={e => setSelectedDept(e.target.value)} className="border px-2 py-1 rounded">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div onClick={() => showModal('오늘 진료 일정', filteredAppointments.filter(a => a.status === '진료중' || a.status === '진료완료'))} className="bg-white p-4 rounded shadow cursor-pointer text-center">
          <div className="text-sm">오늘 진료</div>
          <strong className="text-blue-600 text-xl block mt-1">{todayTreatmentCount}명</strong>
        </div>
        <div onClick={() => showModal('예약 건수 상세', filteredAppointments)} className="bg-white p-4 rounded shadow cursor-pointer text-center">
          <div className="text-sm">예약 건수</div>
          <strong className="text-blue-600 text-xl block mt-1">{todayReserveCount}건</strong>
        </div>
        <div onClick={() => showModal('대기 환자 목록', filteredAppointments.filter(x => x.status === '대기'))} className="bg-white p-4 rounded shadow cursor-pointer text-center">
          <div className="text-sm">대기 환자</div>
          <strong className="text-blue-600 text-xl block mt-1">{todayWaitingCount}명</strong>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl max-h-[80vh] overflow-y-auto">
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {modalData.data.map((item, i) => (
                    <tr key={i}>
                      <td className="border px-3 py-2 text-center">{item.time}</td>
                      <td className="border px-3 py-2 text-center">{item.name}</td>
                      <td className="border px-3 py-2 text-center">{item.title || item.procedure || '-'}</td>
                      <td className="border px-3 py-2 text-center">{item.memo || item.note || '-'}</td>
                      <td className="border px-3 py-2 text-center">{item.department || '-'}</td>
                      <td className="border px-3 py-2 text-center">{item.doctor || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-4 text-right">
              <button onClick={() => setModalData({ ...modalData, visible: false })} className="bg-blue-600 text-white px-4 py-2 rounded text-sm">닫기</button>
            </div>
          </div>
        </div>
      )}
