// src/components/app/waiting/DoctorCard.jsx
  ...
  return (
    <div className="bg-white border-2 border-blue-200 rounded-xl shadow p-4 min-w-[280px] w-full">
      ...
      <div className="text-sm text-blue-800 mb-1">
        <strong>진료과:</strong> {department || '정보 없음'}
      </div>
      {current && (
        <div className="text-sm text-blue-800 mb-1">
          <strong>진료 중:</strong> {current}
        </div>
      )}

      <div className="mt-4">
        <div className="text-blue-700 font-semibold mb-2">
          대기자 목록 ({waiting.length}명)
        </div>
        <ul className="pl-4 text-sm text-gray-800 space-y-1">
          {waiting.length === 0 ? (
            ...
            waiting.map((name, idx) => (
              <li
                key={idx}
                className={idx === 0 ? 'font-bold text-blue-600' : ''}
              >
                {idx + 1}. {name}
              </li>
            ))
